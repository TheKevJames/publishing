...
We have
\begin{align*}
[x^n]{(1-x)}^{-k} &= {\bigg(\frac{1}{1-x}\bigg)}^k\\
...
\begin{proof}
For each composition $(a_1, a_2, \dots, a_k)$ of $n$, $(a_1 -1, a_2 -1, \dots, a_k -1)$ is a ``composition'' of $n-k$, except with partss allowed to be zero. There are ${(n-k)+k-1 \choose k-1} = {n-1 \choose k-1}$ of these by the claim.
\end{proof}
\end{document}

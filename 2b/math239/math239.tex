...
\begin{proof}
Putting there two paths together, we get a walk (not necessarily a path) from $u$ to $w$. By lemma~\ref{walkToPath}, there is a path from $u$ to $w$.
\end{proof}
...
\begin{theorem}[Dirac]
If $G$ is a graph on $n \geq 3$ vertices where every vertex has degree $d \geq \frac{n}{2}$, then $G$ has a cycle containing every vertex (a {\bf Hamiltonian Cycle}).
\end{theorem}

...
A logic consists of \define{syntax}{a format for inscribing the logic} \define{semantics}{the definition of validity within the logic} and \define{proof theories}{methods of determining validity}
...
The semantics are described using \define{boolean valuations}{a function in propositional logic which maps from the set of formulas to the range} We can define these boolean valuations with truth tables. For example, we can map $\true \land \false$ to $\false$.
...
We implicitely use the \define{rule of substitution}{substituting equivalent formulas for sub-formulas} and the \define{rule of transistivity}{all lines are equivalent, not just the closest two}
...
Interpretations are sets of rules our equation must follow. An interpretation must contain a {\bf domain} ($x \isin \{0, 1, 2 \cdots\}$) and a

The {\bf environment} is the mapping between variables and objects within the domain.

For example: $\forall d \isin D : I[x => \text{something}(x,y)], e[x \rightsquigarrow d]$.
...
The {\bf relative image} of $S$ through $R$ is $R(\abs{S}) == \{ y : B \suchthat \exists x : D \cdot (x,y) \in R \land x \in S \}$. Basically, the relative image is the set of elements that are the second element of any pair in $R$ who's first element is in $S$.

We can use the following rules when dealing with relative images: $R(\abs{D \cup B}) == R(\abs{D}) \cup R(\abs{B})$, $R(\abs{D \cap B}) \subseteq R(\abs{D}) \cap R(\abs{B})$, and $R(\abs{dom\ R}) == ran\ R$.

...
A priority queue is a queue for which each element has both a vlue and a non-negative integer priority. \code{leave} means "remove the element of highest/lowest priority, if there are several of the same priority then remove the oldest". This is often used in network routing, since some types of data have higher priority (e.g. streaming video, VoIP, paid QoS).
...
OOP involves \define{declarations}{"this is what it looks like"} and \define{definitions}{"this is what it means"} These can either occur together or the declaration can occur first.

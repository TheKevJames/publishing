...
Then $c = jka$. Since $jk \isin \mathbb{Z}$, we have $a\divides c$.}
...
\subsection*{Subsets}
\definition $S$ is a {\bf subset} of $T$ $(S \subseteq T)$ if and only if for every $x \isin \R{}$, $x \isin S \implies x \isin T$. Note that $S \subseteq S$ and the zero set is a subset of every other set.
...
\proof{
Let $x \isin S \cap (T \cup U)$.\\
So $x \isin S$ and $x \isin T \cup U$.\\
We have two cases:\\
If $x \isin T$, $x \isin S \cap T$.\\
If $x \isin U$, $x \isin S \cap U$.\\
Therefore, $x \isin (S \cap T) \cup (S \cap U)$.}
...
\proposition{$\forall x \isin \R{}, \exists y \isin \R{} \suchthat y < x$}

\proof{
Let $x \isin \R{}$,\\
Then $y = x - 1$ satisfies $y \isin \R{}$ and $y < x$.}

but
\proposition{$\exists y \isin \R{}, \forall x \isin \R{} \suchthat y < x$}

\proof{
If such a $y$ exists, then $x = y$ does not satisfy $y < x$.\\
This is a contradiction, thus the proposition is false.}

\proposition{$\forall x \isin \power{\mathbb{N}}, x \isnotin\!\!\{\} \implies \exists y \isin x, \forall z \isin x \suchthat y < z$}
...
\proposition{For $n \isin \mathbb{Z}$, if $n^2$ is even then $n$ is even}

\proof{
Suppose $n$ is odd.\\
Then $n = 2k + 1$ for some $k \isin \mathbb{Z}$.\\
...
For $n = md, d \isin \mathbb{N}$, we have $1 < d < n$.\\
...
\proposition{$\forall n \geq 60, n = 7x + 11y; n, x, y \isin \mathbb{Z}, x, y \geq 0$. Use the following equalities:
\begin{align}
1 &= 7(-3) + 11(2)\\
1 &= 7(8) + 11(-5)\\
60 &= 7(7) + 11(1)
...
Since $k \geq 67, k - 7 \geq 60$ and so $P(k-7)$ is true, thus \[ k-7 = 7x + 11y \] for some $x$ and $y$. We can manipulate this equation to have \[ k = 7(x + 1) + 11y \] and since $x + 1 \isin \mathbb{Z} \land x + 1 \geq 0, P(k)$.\\
...
Let $a, b, c \isin \mathbb{Z}$. $a \divides b \land a \divides c \implies a \divides bx + cy, \forall x, y \isin \mathbb{Z}$}

\proof{
Since $a \divides b \land a \divides c$, there exists $k, l \isin \mathbb{Z}$ such that $b = ka$ and $c = la$\\
Then $bx + cy = kax + lay = a(kx + ly)$. Since $kx + ly \isin \mathbb{Z}, a \divides bx + cy$.}

\proposition{Bounds by Divisibility\\
Let $a, b \isin \mathbb{Z}. a \divides b \land b \neq 0 \implies \abs{a} \leq \abs{b}$}

\proof{
Since $a \divides b$, there exist $k \isin \mathbb{Z}$ such that $b = ka$.\\
Then $\abs{b} = \abs{ka} = \abs{k}\abs{a} \geq \abs{a}$.}

\subsubsection*{Division Algorithm}
\proposition{Let $a \isin \mathbb{Z} \land b \isin \mathbb{N}$. Then there exist unique integers $q, r$ such that $a = qb + r$ where $0 \leq r < b$}

\subsection*{Greatest Common Divisors}
\definition For any $a, b \isin \mathbb{Z}$ not both $0$, the greatest common divisor of $a$ and $b$, denoted $\gcd(a,b)$, is the integer $d$ such that $d \divides a \land d \divides b$ and $c \divides a \land c \divides b \implies c \leq d$.

\proposition{GCD with Remainders\\
If $a, b, q, r \isin \mathbb{Z}$ such that $a = qb + r$, then $\gcd(a,b) = \gcd(b,r)$}
...
For $a, b \isin \mathbb{Z}$, if $d \divides a, d \divides b, d \geq 0$, and $\exists x, y \isin \mathbb{Z}$ such that $ax + by = d$, then $d = \gcd(a,b)$.}
...
Let $d = \gcd(a,b)$. Then $E(b,r) = k$, so there exists $x_0, y_0 \isin \mathbb{Z}$ such that $bx_0 + ry_0 = \gcd(b,r) = d$.\\
...
\definition For $a,b \isin \mathbb{Z}$, $a$ and $b$ are coprime if $\gcd(a,b) = 1$.

\proposition{Coprimesness and Divisibility\\
If $a, b, c \isin \mathbb{Z}$ where $c \divides ab$ and $a$ and $c$ are coprime, then $c \divides b$.}
...
If $a,b \isin\mathbb{Z}$, $p$ is prime and $p \divides ab$, then $p \divides a$ or $p \divides b$.}
...
If $a,b \isin \mathbb{Z}$ where $d = \gcd(a,b > 0$ then $\gcd(\frac{a}{d},\frac{b}{d}) = 1$}

\subsection*{Linear Diophantine Equations}
\definition An LDE has the form $a_0x_0 + a_1x_1 + ... + a_nx_n = c$ where $a_0, ... a_n, c \isin \mathbb{Z}$ and $x_0, ... x_n$ are integer variables.
...
\proposition{LDE 1\\
Let $a,b,c \isin \mathbb{Z}, d = \gcd(a,b)$. Then $ax + by = c$ has an integer solution if and only if $d \divides c$.}

\proposition{LDE 2\\
Let $a,b,c \isin \mathbb{Z}, d = \gcd(a,b) > 0$. If $(x_0, y_0)$ is an integer solution to $ax + by = c$, the complete set of integers is $\bigl\{ \bigl( x_0 + \frac{b}{d}n, y_0 - \frac{a}{d}n \bigl) \suchthat n \isin \mathbb{Z} \bigl\}$.}
...
\definition Let $m$ be a fixed positive integer and $a,b \isin \mathbb{Z}$. Then $a$ is congruent to $b$ module $m$ if $m \divides a - b$. We write $a \equiv b \pmod m$. Equivelently, $a \equiv b \pmod m$ if $a = b + km$ for some $k \isin \mathbb{Z}$.

Note that congruences are reflexive, symmetric, and transitive.

\subsubsection*{Arithmetic of Cogruences}
\proposition{Let $a,b,a^\prime,b^\prime \isin \mathbb{Z}, m \isin \mathbb{N}$. If $a \equiv a^\prime \pmod m$ and $b \equiv b \pmod m$ then
\begin{itemize}
...
\definition Let $m \isin \mathbb{N}$. For any $a \isin \mathbb{Z}$, we define $[a] = \{ k \isin \mathbb{Z} \suchthat a \equiv k \pmod m \}$. In turn, this allows us to define $\mathbb{Z}_m = \{ [0], [1], ... [m-1] \}$.
...
Since $eD \equiv 1 \pmod{\phi(n)}, \exists k \isin \mathbb{Z}$ such that $ed = 1 + k(p-1)(q-1)$.\\
...
\propositionlist{Transitivity of Divisibility (TD)}{For $a, b \isin \mathbb{Z}$, $a \divides b \land b \divides c \implies a \divides c$}

\propositionlist{Divisibility of Integer Combinations (DIC)}{For $a,b,c,x,y \isin \mathbb{Z}$, $a \divides b \land a \divides c \implies a \divides bx + cy$.}

\propositionlist{Bounds by Divisibility (BBD)}{For $a,b \isin \mathbb{Z}$, $a \divides b \land b \neq 0 \implies \abs{a} \leq \abs{b}$.}

\propositionlist{Division Algorithm (DA)}{For $a,b \isin \mathbb{Z}$ and $b > 0$, $\exists q,r \suchthat a = qb + r \land 0 \leq r < b$.}

\propositionlist{GCD with Remainders (GCD WR)}{For $a,b \isin \mathbb{Z}$ not both zero, if we have integers $q,r$ such that $a = qb + r$ then $\gcd(a,b) = \gcd(b,r)$.}
...
\propositionlist{Extended Euclidean Algorithm (EEA)}{For $a,b \isin \mathbb{Z}$ both positive, then $d = \gcd(a,b)$ can be computed and there exist integers $x$ and $y$ such that $ax + by = d$.}
...
\propositionlist{Existence of Inverses is $\mathbb{Z}_p$ (INV $\mathbb{Z}_p$)}{If $p$ is prime and $[a]$ is a non-zero element within $\mathbb{Z}_p$, then $\exists [b] \isin \mathbb{Z}_p \suchthat [a] \cdot [b] = 1$}
...
\propositionlist{Coprimeness and Divisibility (CAD)}{Let $a,b,c \isin \mathbb{Z}$. If $a$ and $c$ are coprime, $c \divides ab \implies c \divides b$}

\propositionlist{Division by GCD (DB GCD)}{For $a,b \isin \mathbb{Z}$, not both zero, $d = \gcd(a,b) \implies 1 = \gcd(\frac{a}{d},\frac{b}{d})$}

\propositionlist{Linear Diophantine Equation Theorem 1 (LDET1)}{Let $a,b,c \isin \mathbb{Z}$ and $d = \gcd(a,b)$. $ax + by = c$ has an integer solution if and only if $d \divides c$}

\propositionlist{Linear Diophantine Equation Theorem 2 (LDET2)}{Let $a,b,c \isin \mathbb{Z}$ and $d = \gcd(a,b) \neq 0$. If $(x_0,y_0)$ is one particular integer solution to $ax + by = c$, then the complete set of integer solutions is \[ \bigg( x_0 + \frac{b}{d}n, y_0 - \frac{a}{d}n \bigg) \suchthat n \isin \mathbb{Z} \]}
...
\propositionlist{Congruence is an Equivalence Relation (CER)}{Let $m \isin \mathbb{N}$ and $a,b,c \isin \mathbb{Z}$. Then $a \equiv a \pmod m$, $a \equiv b \pmod m \implies b \equiv a \pmod m$, and $a \equiv b \pmod m \land b \equiv c \pmod m \implies a \equiv c \pmod m$}
...
\propositionlist{Congruent iff Same Reminder (CISR)}{Let $a,b \isin \mathbb{Z}, m \isin \mathbb{N}$. Then $a \equiv b \pmod m \iff a \% m = b \% m$}
...
\propositionlist{Properties of Modulus (PM)}{If $z, w \isin \mathbb{C}$, then $\abs{z} = 0 \iff z = 0$, $\abs{z} = \abs{\overline{z}}$, $\abs{z}^2 = z\overline{z}$, $\abs{zw} = \abs{z}\abs{w}$, $\abs{z + w} \leq \abs{z} + \abs{w}$}

\propositionlist{DeMoivre's Theorem (DMT)}{For any $\theta \isin \mathbb{R}$ and $n \isin \mathbb{Z}$, $(\cos\theta + i\sin\theta)^n = \cos n\theta + i\sin n\theta$}
...
\propositionlist{Fundamental Theorem of Algebra (FTA)}{For all complex polynomials, $f(x)$ with $\deg(f(x)) \geq 1$, $\exists x_0 \isin \mathbb{C} \suchthat f(x_0) = 0$}
...
\propositionlist{Conjugate Roots Theorem (CRT)}{Let $f(x) \isin \mathbb{R}[x]$, if $c \isin \mathbb{C}$ is a root of $f(x)$, then so is $\overline{c}$}

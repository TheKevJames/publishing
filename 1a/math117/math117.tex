...
&= x - \frac{5}{2}\ln\abs{x+3} + \frac{1}{2}\ln\abs{x-1} + C
\end{align*}
...
\item $\inint{\frac{dx}{x}}{x} = \ln\abs{x} + C$
...
\item $\inint{\tan x}{x} = -\ln \abs{\cos x} + C = \ln \abs{\sec x} + C$ through {\bf substitution}
\item $\inint{\sec x}{x} = \ln \abs{\sec x + \tan x} + C$ through {\bf hell if I know}
\end{itemize}
...
\textit{Definition:} The {\bf area between curves} $f(x)$ an $g(x)$ we essentially create rectangles that fit between $f(x)$ an $g(x)$. The base of each rectangle is still dx, but the height is $\abs{f(x) - g(x)}$
...
\begin{align*}
\dint{0}{1}{x}{x} + \dint{1}{2}{\frac{2}{x} - 1}{x} &= \frac{x^2}{2}\bigg|_0^1 + (2\ln\abs{x} - x)\bigg|_1^2\\
...
\begin{align*}
\dint{0}{1}{\frac{2}{y+1} - y}{y} &= \bigg(2\ln\abs{y+1} - \frac{y^2}{2}\bigg)\bigg|_0^1\\
&= 2\ln 2 - \frac{1}{2}
\end{align*}
...
Example: consider the curve $y = \frac{2}{3}x^{\frac{3}{2}}$. Find its length for $x\isin [0,3]$
...
&= \lim_{t\to\infty} \ln\abs{x} \bigg|_1^t\\
&= \infty
\end{align*}
...
To use {\bf polar coordinates} instead of cartesian coordinates $(x,y)$, we specify $(r,\theta)$ where r is the distance between the point and the origin, usually $r \isin [0,\infty]$, and $\theta$ is the angle between the positive x axis and the point, where $\theta \isin [0,2\pi]$
...
It is also common to write {\bf complex numbers} in polar form. So, if $z = a + bi$, we have \[ z = r(\cos\theta + i\sin\theta) \] $r$ is called the {\it modulus} of $z$, and is written as $\abs{z}$. $\theta$ is the {\it argument} of $z$.
